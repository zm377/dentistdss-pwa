 ...
 */
export const getWelcomeMessage = (chatType, userRole, currentUser) => {
 ...
 */
export const getDashboardWelcomeMessage = (userRole, currentUser, clinicDetails) => {
  switch (userRole) {
    case 'DENTIST':
 ...
 */
export const getQuickActions = (chatType, setQuickInput) => {
  const actions = {
    receptionist: [
      {
...
export const formatAppointmentTime = (appointment) => {
  return appointment.startTime || appointment.time || 'Time TBD';
};

/**
 * Format appointment display text for different user roles
 */
export const formatAppointmentText = (appointment, userRole) => {
  if (userRole === 'DENTIST') {
 ...
 */
export const getAppointmentServiceType = (appointment) => {
  return appointment.serviceType || appointment.type || 'General Appointment';
};

/**
 * Common loading component props
 */
export const getLoadingProps = (message = 'Loading...') => ({
  sx: { p: 3 },
  children: [
 ...
 */
export const getErrorProps = (error) => ({
  severity: 'error',
  sx: { mb: 2 },
  children: error
});
